...
const practice3 = traders
    .filter(user => user.trader.city.includes(`대전`))
    .map(user => ({
        name: user.trader.name,
        city: user.trader.city
    }));
...
const practice5 = traders
    .filter(user => user.trader.city.includes(`서울`))
    .map(user => user.value);

let total = 0;
practice5.forEach(value => {
    total += value;
});
