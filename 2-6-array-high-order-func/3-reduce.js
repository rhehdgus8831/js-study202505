...
function myReduce(callback, initialValue) {
    // 누산기(accumulator)를 초기화함
    // 초기값(initialValue)이 주어졌으면 그 값을 사용하고,
    // 아니면 appleBasket 배열의 첫 번째 요소를 사용
    let accumulator = initialValue !== undefined ? initialValue : appleBasket[0];

    // 반복을 시작할 인덱스 설정
    // 초기값이 있으면 인덱스 0부터, 없으면 1부터 시작 (왜냐면 첫 값을 이미 누산기로 썼으니까!)
    const startIndex = initialValue !== undefined ? 0 : 1;

    // appleBasket 배열을 순회하면서 누산 작업을 수행
    for (let i = startIndex; i < appleBasket.length; i++) {
        // callback 함수에 현재 누산된 값과 현재 요소를 넘겨서 결과를 누산기(accumulator)에 다시 저장
