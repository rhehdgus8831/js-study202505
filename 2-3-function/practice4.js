...
function calculateAverageScore(...scores) {
    ...
    // 원본 배열을 변경하지 않기 위해 배열 복사
    const tempScores = [...scores];

    // 1. 최솟값과 최댓값 찾기
    let minScore = tempScores[0];
    let maxScore = tempScores[0];

    for (let i = 1; i < tempScores.length; i++) {
        if (tempScores[i] < minScore) {
            minScore = tempScores[i];
        }
        if (tempScores[i] > maxScore) {
            maxScore = tempScores[i];
        }
    }

    // 2. 최고점과 최저점을 하나씩 배열에서 제거 (splice 사용)
    const maxIndex = tempScores.indexOf(maxScore);
    if (maxIndex > -1) {
        tempScores.splice(maxIndex, 1);
    }

    const minIndex = tempScores.indexOf(minScore);
    if (minIndex > -1) {
        tempScores.splice(minIndex, 1);
    }

    const validScores = tempScores;

    // 3. 유효한 점수가 2개 미만이면 평균 계산 의미 없음
    if (validScores.length < 2) {
        return 0;
    }

    // 4. 유효 점수들의 합계 및 평균 계산
    let sum = 0;
...
function countChar(words, count) {
    let counts = 0;
